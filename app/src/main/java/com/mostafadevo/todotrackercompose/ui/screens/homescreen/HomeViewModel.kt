...
constructor(
  ...
  val addTodoDialogUiState =
    _addTodoDialogUiState.stateIn(
      scope = viewModelScope,
      started = SharingStarted.WhileSubscribed(5000),
  ...
  val todoDetailesBottomSheetUiState =
    _todoDetailesBottomSheetUiState.stateIn(
      scope = viewModelScope,
      started = SharingStarted.WhileSubscribed(5000),
  ...
  private fun loadTodos() {
    viewModelScope.launch {
      _homeUiState.debounce(debouncePeriod).collectLatest { state ->
        ...
        val todosFlow =
          when (state.selectedSegmentIndex) {
            0 -> {
  ...
  fun onDialogEvent(events: AddTodoDialogUiEvents) {
    when (events) {
      ...
      is AddTodoDialogUiEvents.OnDateChange -> {
        _addTodoDialogUiState.value =
          _addTodoDialogUiState.value.copy(
            date = events.date,
          )
      ...
      is AddTodoDialogUiEvents.OnTimeChange -> {
        ...
        Timber.d("current time is ${events.time}")
      }
